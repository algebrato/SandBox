...
Packet::Packet(const uint8_t *data, size_t size, bool /*doAllign*/)
    : Packet()
{
    auto pkt_data = MediaWrapper::AV::memdup<uint8_t>(data, size);
    if (!pkt_data) {
        return;
    }
...
bool Packet::setData(const uint8_t *newData, size_t size, OptionalErrorCode ec)
{
    ...
    auto data = MediaWrapper::AV::memdup<uint8_t>(newData, size);
    if (!data) {
        throws_if(ec, AVERROR(ENOMEM), ffmpeg_category());
        return false;
    }
...
{
    return {raw()->pts != MediaWrapper::AV::NoPts ? raw()->pts : raw()->dts, m_timeBase};
}
