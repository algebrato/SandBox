...
int custom_io_read(void *opaque, uint8_t *buf, int buf_size)
{
    if (!opaque)
        return -1;
    MediaWrapper::AV::CustomIO *io = static_cast<MediaWrapper::AV::CustomIO*>(opaque);
...
Stream FormatContext::stream(int idx)
{
...
Stream FormatContext::stream(int idx, OptionalErrorCode ec)
{
    if (!m_raw) {
...
{
    return addStream(static_cast<const CodecContext&>(encCtx), ec);
}
...
void FormatContext::seek(const Timestamp& timestamp, int streamIndex, OptionalErrorCode ec)
{
...
void FormatContext::seek(const Timestamp &timestamp, int streamIndex, bool anyFrame, OptionalErrorCode ec)
{
...
Timestamp FormatContext::duration() const noexcept
{
    ...
    if (m_raw->duration != MediaWrapper::AV::NoPts) {
        duration += (m_raw->duration <= INT64_MAX - 5000 ? 5000 : 0);
    }

    return {duration, MediaWrapper::AV::TimeBaseQ};
}
...
bool FormatContext::checkUncodedFrameWriting(int streamIndex, error_code &ec) noexcept
{
    ec.clear();
...
bool FormatContext::checkUncodedFrameWriting(int streamIndex) noexcept
{
    error_code ec;
    return checkUncodedFrameWriting(streamIndex, ec);
}

void FormatContext::writeUncodedFrame(VideoFrame &frame, int streamIndex, OptionalErrorCode ec)
{
    writeFrame(frame.raw(), streamIndex, ec, av_interleaved_write_uncoded_frame);
}

void FormatContext::writeUncodedFrameDirect(VideoFrame &frame, int streamIndex, OptionalErrorCode ec)
{
    writeFrame(frame.raw(), streamIndex, ec, av_write_uncoded_frame);
}

void FormatContext::writeUncodedFrame(AudioSamples &frame, int streamIndex, OptionalErrorCode ec)
{
    writeFrame(frame.raw(), streamIndex, ec, av_interleaved_write_uncoded_frame);
}

void FormatContext::writeUncodedFrameDirect(AudioSamples &frame, int streamIndex, OptionalErrorCode ec)
{
    writeFrame(frame.raw(), streamIndex, ec, av_write_uncoded_frame);
}
...
Stream FormatContext::addStream(const CodecContext &ctx, OptionalErrorCode ec)
{
    clear_if(ec);
    if (!m_raw)
    {
